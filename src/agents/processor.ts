...
export async function vectorSearchWithDetails(userQuery: string, topK = 3): Promise<Candidate[]> {
...
function extractKeywords(text: string): string[] {
  const stop = new Set([
    'i','need','a','an','the','for','with','and','or','of','to','me','my','is','there','something','better','about','on','in','it','this','that','looking','want','would','like'
  ]);
  return text
    .toLowerCase()
    .replace(/[^a-z0-9\s]/g, ' ')
...
export async function findRelevantProducts(
  ...
  const filtered = (matches || []).filter(m => typeof m.score === 'number' && (m.score as number) >= minScore);
  const ids = filtered.map(m => m.id);
  logger.info({ userQuery, minScore, ids, scores: filtered.map(m => m.score) }, 'Processor: preFetch relevant');
  if (ids.length === 0) return [];
  const products = await getProductsByIds(ids);
  const keywords = extractKeywords(userQuery);
  const relevant = products.filter(p => isRelevant(p, keywords)).slice(0, topK);
...
export async function pickBestOrNA(userQuery: string, candidates: Candidate[]): Promise<ProcessorPickDecision> {
  const system = [
  ...
  if (parsed?.not_available === true) return { not_available: true, reason: parsed?.reason || 'not suitable' };
  if (parsed?.best_product_id) return { best_product_id: parsed.best_product_id, rationale: parsed?.rationale };
