...
export type Mode = 'SMALL_TALK' | 'FOLLOWUP_QA' | 'MORE_PRODUCTS' | 'NEW_PRODUCT' | 'NOT_AVAILABLE';
...
export async function decideFlowLLM(input: {
  ...
  const fewshot = [
    ...
    { role: 'user', content: JSON.stringify({ user_query: 'hi', has_last_product: false, has_last_candidates: false }) },
    { role: 'assistant', content: '{"mode":"SMALL_TALK"}' },
    { role: 'user', content: JSON.stringify({ user_query: 'is there something better?', has_last_product: true, has_last_candidates: true }) },
    { role: 'assistant', content: '{"mode":"MORE_PRODUCTS"}' },
    { role: 'user', content: JSON.stringify({ user_query: 'what is the price?', has_last_product: true, has_last_candidates: false }) },
    { role: 'assistant', content: '{"mode":"FOLLOWUP_QA"}' },
    { role: 'user', content: JSON.stringify({ user_query: 'I need a foot massager', has_last_product: false, has_last_candidates: false }) },
    { role: 'assistant', content: '{"mode":"NEW_PRODUCT"}' },
    // New-product vs more-products disambiguation
    { role: 'user', content: JSON.stringify({ user_query: 'And a neck massager?', has_last_product: true, has_last_candidates: true, last_product: { category: 'Healthtech and Wellness', product_name: 'Revive Foot & Leg Massager' } }) },
    { role: 'assistant', content: '{"mode":"NEW_PRODUCT"}' },
...
export async function fetchProductsByIds(ids: string[]): Promise<ProductRow[]> {
  ...
  const r = await query<ProductRow>(`SELECT * FROM products WHERE id IN (${params})`, ids);
