...
import {
  saveMessage,
  getLastAssistantProduct,
  getLastAssistantCandidates,
  getRecentTurns,
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    logger.info(
      {
        hasLastProduct: !!lastProduct,
        lastCandidatesIdsCount: lastCandidatesIds?.length || 0,
        recentMessages,
      },
      "advice.context"
    );
    // Deciding mode with supervisor LLM
      ...
      const stream = await llm.stream([
        {
          role: "system",
          content:
            "You are a friendly assistant. Keep it brief. Ask what product the user is looking for.",
        },
    ...
    if (mode === "FOLLOWUP_QA" && lastProduct) {
      ...
      const stream = await llm.stream([
        {
          role: "system",
          content:
            "Answer ONLY using provided selected_product fields. If unknown, say you are not sure. Be concise.",
        },
        {
          role: "user",
          content: JSON.stringify({
            user_query: userQuery,
            selected_product: lastProduct,
      ...
      logger.info(
        { mode, productId: lastProduct.id, replyLength: reply.length },
        "advice.final"
      );
      sseSend(
        { rationale: reply, product: lastProduct, mode: "FOLLOWUP_QA" },
        "final"
      );
      return sseClose();
    }
    ...
    if (mode === "MORE_PRODUCTS" && lastProduct) {
      ...
      const baseIds = lastCandidatesIds?.length
        ...
        : (
            await top3ByQuery(
              `${lastProduct.category} ${lastProduct.product_name}`
            )
          ).map((p) => p.id);

      const remainingIds = baseIds
        .filter((id) => id !== lastProduct.id)
        .slice(0, 2);
      ...
      const stream = await llm.stream([
        {
          role: "system",
          content:
            "Briefly compare these alternatives to the previously suggested product. Be concise.",
        },
        {
          role: "user",
          content: JSON.stringify({
            previous_product: lastProduct,
            candidates,
          }),
        },
      ]);
      ...
      logger.info(
        { mode, candidateIds: candidates.map((c) => c.id) },
        "advice.final"
      );
    ...
    let top3 = await findRelevantProducts(userQuery, 3, 0.7);
    if (!top3.length) {
      ...
      const stream = await llm.stream([
        {
          role: "system",
          content:
            "Politely tell the user the requested product is not available right now, and that it will be added soon. Ask if they want suggestions from related categories.",
        },
        { role: "user", content: userQuery },
      ]);
      for await (const chunk of stream) {
    ...
    if (pick.not_available === true) {
      ...
      logger.info(
        { mode: "NOT_AVAILABLE", reason: pick.reason },
        "advice.final"
      );
    ...
    const stream = await llm.stream([
      {
        role: "system",
        content:
          "Provide a concise rationale for why this product fits the user request.",
      },
      {
        role: "user",
        content: JSON.stringify({
